...
// TestAddServiceAppliesSubmittedDeviceThreshold guards step 2's save: the
// root path and threshold fields submitted alongside the service must
// result in that device's thresholds being persisted, resolved
// server-side (not trusting the client's earlier device-for-path lookup).
func TestAddServiceAppliesSubmittedDeviceThreshold(t *testing.T) {
	radarrSrv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte(`{"version":"5.0.0"}`))
	}))
	defer radarrSrv.Close()
	...
	rootPath := t.TempDir()
	body, contentType := multipartServiceForm(t, map[string]string{
		"type": "radarr", "name": "Movies", "url": radarrSrv.URL, "api_key": "key",
		"root_path": rootPath, "target_usage_percent": "80", "critical_usage_percent": "88",
	})
	recorder := httptest.NewRecorder()
	request := httptest.NewRequest(http.MethodPost, "/services/create", body)
	request.Header.Set("Content-Type", contentType)
	...
	target, critical := reloaded.ThresholdsFor(rootPath)
	if target != 80 || critical != 88 {
		t.Fatalf("expected the submitted thresholds to be persisted for %s, got %v/%v", rootPath, target, critical)
	}
}
