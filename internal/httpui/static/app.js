...
(() => {
  ...
  var ShellController = class extends window.Stimulus.Controller {
    ...
    click(event) {
      ...
      const cleanupProtect = target.closest("[data-cleanup-protect]");
      if (cleanupProtect) {
        event.preventDefault();
        this.protectCleanupAction(cleanupProtect);
      }
      const cleanupExclude = target.closest("[data-cleanup-exclude]");
      if (cleanupExclude) {
        event.preventDefault();
    ...
    // actually displays.
    currentExcludedKeys(button) {
      const form = button.closest("form");
      if (!form) return [];
      return [...form.querySelectorAll('input[name="excluded"]')].map((input) => input.value);
    }
    ...
    reopenCleanupPlanExcluding(button, key) {
    ...
    async protectCleanupAction(button) {
      const path = button.dataset.path;
      const key = button.dataset.key;
      if (!path || !key) return;
      button.disabled = true;
      try {
        ...
        announce(`Protect failed: ${error.message}`);
        button.disabled = false;
      }
    }
