(() => {
  ...
  application.register("revisions", class extends window.Stimulus.Controller {
    connect() {
      this.etag = "";
      this.pollTimer = null;
      this.onVisible = () => {
  ...
  application.register("updates", class extends window.Stimulus.Controller {
    apply() {
  ...
  application.register("dashboard", class extends window.Stimulus.Controller {
	connect() {
	  this.etag = "";
  ...
  application.register("shell", class extends window.Stimulus.Controller {
    connect() {
    ...
    click(event) {
      const removal = event.target.closest("[data-removal-url]");
      if (removal) {
        event.preventDefault();
        this.openRemoval(removal.dataset.removalUrl, removal);
        return;
      }
      const loadingCancel = event.target.closest("[data-modal-cancel-loading]");
      if (loadingCancel) {
        event.preventDefault();
        if (this.modalRequest) this.modalRequest.abort();
        this.closeModal();
  ...
  application.register("removal", class extends window.Stimulus.Controller {
