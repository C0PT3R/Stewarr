...
func (service *Service) SetRemovalSettings(autoMode string, autoRemoveUnassociated, dryRun bool, torrentCarePercent float64) error {
	service.configMu.Lock()
	defer service.configMu.Unlock()
	...
	updatedCfg, err := config.SetRemovalSettings(currentCfg, autoMode, autoRemoveUnassociated, dryRun, torrentCarePercent)
	if err != nil {
		return err
	}
