...
func TestTargetedReconciliationUpdatesBaseFingerprint(t *testing.T) {
	root := t.TempDir()
	removedPath := filepath.Join(root, "removed.mkv")
	preservedPath := filepath.Join(root, "preserved.mkv")
	if err := os.WriteFile(removedPath, []byte("removed"), 0o644); err != nil {
	...
	service.mu.Lock()
	service.generation = 2
	// Simulate the exact bug scenario: a prior full Refresh() left a stale
	// fingerprint baseline in place (as if computed before this removal).
	service.baseFingerprint = [32]byte{0xff}
	...
	service.mu.RLock()
	got := service.baseFingerprint
	items, torrents := service.items, service.torrents
	service.mu.RUnlock()
	want := inventoryFingerprint(items, torrents)
	if got != want {
		t.Fatalf("targeted reconciliation left a stale base fingerprint: got %x, want %x — the next periodic Refresh() would wrongly flip FileModel back to stale", got, want)
	}
}
