...
func TestTrackerHealthWorkingWhenAnyTrackerIsConfirmedWorking(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "mixed":
			_ = json.NewEncoder(w).Encode([]any{
				map[string]any{"status": 4, "msg": "Connection timed out"},
				map[string]any{"status": 2, "msg": ""},
			})
		case "allfailing":
			_ = json.NewEncoder(w).Encode([]any{
				map[string]any{"status": 4, "msg": "Unregistered torrent"},
			})
		case "none":
			_ = json.NewEncoder(w).Encode([]any{})
		default:
	...
	defer srv.Close()
	client := New("qBittorrent", srv.URL, "", "", "token")
	out, err := client.TrackerHealth(map[string]model.Torrent{
		"mixed":      {Hash: "mixed"},
		"allfailing": {Hash: "allfailing"},
		"none":       {Hash: "none"},
	})
	if err != nil {
		t.Fatal(err)
	}
	if !out["mixed"].Working || !out["mixed"].Known {
		t.Fatalf("expected mixed (one working tracker) to report Working=true, Known=true: %#v", out["mixed"])
	}
	if out["allfailing"].Working || !out["allfailing"].Known || out["allfailing"].Message == "" {
		t.Fatalf("expected allfailing to report Working=false, Known=true, with a message: %#v", out["allfailing"])
	}
	if out["none"].Known {
		t.Fatalf("expected a torrent with no tracker entries to report Known=false, got %#v", out["none"])
	}
}

func TestTrackerHealthToleratesOneTorrentRemovedDirectlyInQBittorrent(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Query().Get("hash") == "gone" {
			w.WriteHeader(http.StatusNotFound)
			_, _ = w.Write([]byte("Not Found"))
